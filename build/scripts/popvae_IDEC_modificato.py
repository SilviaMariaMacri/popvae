...
class IDEC(object):
    ...
    def clustering(self, x, y=None,
        ...
        loss = [0, 0]#, 0]
        index = 0
        for ite in range(1):#int(maxiter)):
            print('ite: ',ite)
            ...
            self.model.load_weights(out+"/euromds_weights.hdf5")
            pred=self.encoder.predict(x,batch_size=self.batch_size) #returns [mean,sd,sample] for individual distributions in latent space
            p=pd.DataFrame()
            p['mean1']=pred[0][:,0]
            p['mean2']=pred[0][:,1]
            p['sd1']=pred[1][:,0]
...
if __name__ == "__main__":
    ...
    parser.add_argument('--save_dir', default='results/idec_var3')
