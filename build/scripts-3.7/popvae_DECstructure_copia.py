...
parser.add_argument("--out",default="out/dec_structure23",
...
dims=[54, 500, 500, 2000, 10]
vae = autoencoder(dims)

def autoencoder_variato(dims):
    ...
    #decoder
    #decoder_input=layers.Input(shape=(latent_dim,),name='z_sampling')
    x=layers.Dense(2000,activation="relu",name='d_2')(z)#was elu
    x=layers.Dense(500,activation="relu",name='d_1')(x)
    x=layers.Dense(500,activation="relu",name='d_0')(x)
    output=layers.Dense(dims[0],activation="sigmoid")(x) #hard sigmoid seems natural here but appears to lead to more left-skewed decoder outputs.
    #decoder=Model(decoder_input,output,name='decoder')
    ...
    #end-to-end vae
    #output_seq = decoder(encoder(input_seq)[2])
    modello = Model(input_seq, output, name='vae_variato')
    
    return modello
    
#vae = autoencoder_variato(dims)
...
vae.compile(loss={'dense_13':loss_autoencoder},optimizer=opt)
